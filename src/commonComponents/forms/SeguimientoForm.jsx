...
const SeguimientoForm = ({ formData, handleChange, handleSubmit, errors, onFinalizar }) => {
  ...
  const handleViewDetails = (viaje) => {
    setDetailModal({
      open: true,
      title: `Detalles del Viaje: #${(viaje.numeroViaje || viaje._id).toString().slice(-6)}`,
      content: (
        <Box sx={{ p: 2 }}>
          <Typography variant="subtitle1" gutterBottom>Información del Viaje</Typography>
          <Divider sx={{ my: 1 }} />
          <Stack spacing={1}>
            <Typography><strong>Número:</strong> #{(viaje.numeroViaje || viaje._id).toString().slice(-6)}</Typography>
      ...
      <SelectionModal
        ...
        onSelect={(viaje) => {
          ...
          handleChange({
            target: {
              name: 'numeroViaje',
              value: (viaje.numeroViaje || viaje._id).toString().slice(-6)
            }
          });
        ...
        loading={loading}
        getText={(viaje) => `Viaje #${(viaje.numeroViaje || viaje._id).toString().slice(-6)}`}
                  ...
                  <TableBody>
                    {historialPaginado.map((item, index) => (
                      <TableRow key={index} hover>
                        ...
                        <TableCell>#{item.numeroViaje ? item.numeroViaje.toString().slice(-6) : item._id.toString().slice(-6)}</TableCell>
